...
- [specs/web-app-structure.md](specs/web-app-structure.md): Web 端当前对齐 Norn 移动端后的主壳、三 tab 信息架构、详情流、Sequence 交互和 feature 边界真相源，并同步记录 Web 当前继续承担桌面优先的系统化编辑工作流、与 iOS 一致的 `Sequence / Task Pool / Schedule` 顺序、壳顶部全局可见但已收敛为标题 + 设置入口的主壳、path 驱动的顶层路由状态、显式收口的 Sequence 编辑态、可触发的 Quick Add 次级动作，以及向 iOS `NornTheme` 靠拢的背景 / card / pill 分层语义。最后更新：2026-04-04
