...
- [specs/web-app-structure.md](specs/web-app-structure.md): Web 端当前对齐 Norn 移动端后的主壳、三 tab 信息架构、详情流、Sequence 交互和 feature 边界真相源，并同步记录 Web 当前继续承担桌面优先的系统化编辑工作流、与 iOS 一致的 `Sequence / Task Pool / Schedule` 顺序、设置页承载的外观模式与设备会话管理，以及脑图节点布局和折叠状态的持久化。最后更新：2026-04-04
...
- [guides/implementation-lessons.md](guides/implementation-lessons.md): 项目级实施教训真相源。记录用户纠错后的具体教训，以及生成方案前后的 lesson review 工作流，并补充 `Sequence` route B 下不要再赌 `ScrollView` 行内系统 `swipeActions`、工程入口迁移后要同步收口脚本与入口文档并收敛到真实工作流入口、以及 roadmap 确认后必须按节点即时提交的约束。最后更新：2026-04-04
