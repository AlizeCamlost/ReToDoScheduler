...
- [runbooks/ios.md](runbooks/ios.md): iOS 原生工程打开、真机安装、日常运行和最小同步配置，并补充 `任务池` 现位于第二个顶层分页入口，默认展示 `目录` 并可切换到 `脑图`，同步状态与“隐藏已完成任务”开关统一位于设置页，目录页已收敛为上半屏导航树 + 下半屏当前目录目的地，`Sequence` 当前序列则需长按进入编辑态后才支持拖拽与左滑动作。最后更新：2026-04-01
