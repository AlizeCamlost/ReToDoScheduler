...
- [runbooks/server-deploy.md](runbooks/server-deploy.md): API + PostgreSQL 的首轮部署、校验、日常运维、备份和 GitHub Actions 部署说明，并记录当前生产部署绕过 `docker compose build`、改用 `DOCKER_BUILDKIT=0 docker build` 的过渡性约束，以及 Web owner 登录环境变量、同源 session cookie、默认 `Secure` 但可通过 `WEB_SESSION_COOKIE_SECURE=false` 临时退回 HTTP-only 部署且需同步传入 `api` 容器的登录边界与 migration runner 约束。最后更新：2026-04-04
