...
- [specs/web-app-structure.md](specs/web-app-structure.md): Web 端当前对齐 Norn 移动端后的主壳、三 tab 信息架构、详情流、Sequence 交互和 feature 边界真相源，并同步记录 Web 运行时同步设置、显式进入的 Sequence 编辑态、7 项当前序列上限、Task Pool 的 `目录 / 脑图` 双模式、目录 CRUD / 任务归位，以及脑图节点布局和折叠状态的持久化。最后更新：2026-04-02
