...
7. 同步成功后，第二页默认会显示目录树浏览器；顶部 segmented control 可以切到 `画布`
8. `任务池` 顶部提供 `隐藏已完成任务` 开关；打开后，目录树和画布都会统一隐藏已完成但未归档的任务
9. 在目录树里长按目录行可以新建子目录、重命名、移动或删除目录；在画布里拖拽节点会直接写回布局
10. 画布支持双指缩放，也可以用右上角的缩放控件放大、缩小或重置到 `100%`
