...
9. 在 `目录` 视图里，上半屏是可折叠的目录导航树，下半屏是可折叠子目录的当前目录目的地；内容会与屏幕边缘保留必要留白，点击整个目录行即可选中目录，导航树中的父子目录会通过更明显的缩进表达层级，展开箭头会紧挨文件夹图标，右侧数字保持右对齐，而目的地下的子目录保持不缩进的平铺列表，并在列表顶部额外提供 `..` 返回上级目录
