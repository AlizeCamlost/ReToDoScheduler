...
>
> 最后更新：2026-03-08（Phase 3 首轮完成 + Migration 系统上线）
...
| # | 任务 | 状态 | 备注 |
|---|------|------|------|
| 3.R2.1 | 子任务支持（task_parts） | ⬜ | 需 API 新增 CRUD 端点 + 前端 UI |
| 3.R2.2 | 确定下一批功能清单 | ⬜ | |
...
1. **Phase 3 继续**：
   - 子任务功能（`task_parts`，需 API 端新增端点 + migration + 前端 UI）
   - 规划下一批 Web 端功能
2. **跨阶段高优先级**：
   - X.1 + X.2：CI 保护 deploy，避免坏代码部署到生产
   - X.3：域名到位后立即配置 HTTPS（Phase 4B 前置依赖）
3. 本地开发环境已就绪，启动命令见上方速查表
4. Migration 系统已就绪，schema 变更按流程添加 `.sql` 文件即可
5. 关注域名审批进度（预计 ~03-11），到位后立即配置 HTTPS
