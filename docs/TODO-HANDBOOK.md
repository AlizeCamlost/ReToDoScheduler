...
| 2.8 | 确认 Web 的环境变量配置正确 | ⬜ | `VITE_API_BASE_URL`(本地默认空), `VITE_API_AUTH_TOKEN` |
...
1. **Phase 2**：搭建本地开发环境——启动本地 DB + API + Web，跑通联调
2. 本地开发环境的 `docker-compose.yml`（开发用）中的数据库容器名也是 `retodo-db`，注意与生产 compose 区分
3. 本地 Web 需要配置 `VITE_API_AUTH_TOKEN` 环境变量才能与 API 通信
