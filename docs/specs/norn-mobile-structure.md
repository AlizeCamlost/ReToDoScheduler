...
- `App/NornApp.swift` 已承担 live 依赖组装，`UI/Root/ContentView.swift` 已承担共享背景、bottom+horizontal edge-to-edge 分页裁剪壳、Sequence page scoped safeAreaInset 和 sheet 挂载点
...
- `Sequence` 底部输入 dock 已回到 page scoped `safeAreaInset` 语义，不再通过手算 reserve 和内容 padding 模拟安全区
- `Sequence` 的滚动裁剪范围继续由 root page shell 的 bottom edge-to-edge 延伸承担，不与 dock 的 safe-area 语义混用
...
| `UI/Sequence/SequenceTab.swift` | `SequenceTab` | 当前序列页 | 当前聚焦 + 主序列 + 接下来摘要，主序列通过长按卡片重排，时间线使用连续轨道 |
