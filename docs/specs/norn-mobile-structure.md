...
- `Domain/Legacy/Models.swift` 是拆分前的过渡停放点，不是长期结构
- `UI/` 已经按页面和共享组件分层
...
flowchart LR
  F0[文档与类型地图]
  F1[Task Domain Skeleton]
  F2[Local Persistence Boundary]
  F3[Sync Boundary]
  F4[Application Store Shell]
  F5[Quick Add]
  F6[Task Detail]
  F7[Task Editing]
  F8[Sync Settings And Manual Sync]
  F9[TaskPool Managed List]
  F10[Test Harness]
...
| --- | --- | --- | --- |
| `F0` | 固定目录、类型地图和 feature 拓扑 | `docs/specs/norn-mobile-structure.md` | `docs(norn): define mobile structure and feature topology` |
| `F0.1` | 完成目录骨架整理与过渡停放 | `App/` `Resources/` `Domain/Legacy/Models.swift` | `refactor(norn): reorganize mobile directories for semantic layering` |
| `F1` | 从 `Domain/Legacy/Models.swift` 拆出 Task 领域模型骨架 | `Domain/App` `Domain/Task` | `refactor(norn): define task domain skeleton` |
| `F2` | 拆出本地持久化边界与实现壳 | `Infrastructure/Persistence` | `refactor(norn): define local persistence boundary` |
| `F3` | 拆出同步边界与 DTO 壳 | `Infrastructure/Sync` | `refactor(norn): define sync boundary` |
| `F4` | 建立 `NornAppStore` 和用例壳 | `Application/State` `Application/UseCases` | `refactor(norn): add application state shell` |
| `F5` | 接通 Quick Add 本地新增 | `UI/Root` `UI/Shared` `Application/UseCases/QuickAddTaskUseCase.swift` | `feat(norn): add quick add task creation flow` |
| `F6` | 接通详情查看与状态动作 | `UI/Sequence` `UI/TaskPool/TaskDetailSheet.swift` | `feat(norn): add task detail flow` |
| `F7` | 接通任务编辑器和保存 | `UI/TaskPool/TaskEditorSheet.swift` `Domain/Task/TaskDraft.swift` | `feat(norn): add task editing flow` |
| `F8` | 接通同步设置与手动同步 | `UI/Settings` `Application/UseCases/SyncTasksUseCase.swift` | `feat(norn): add sync settings and manual sync` |
| `F9` | 让 TaskPool 的 `list` 模式接入真实数据 | `UI/TaskPool/TaskPoolTab.swift` | `feat(norn): connect task pool list to app state` |
| `F10` | 加上数据流和 UI smoke tests | `NornTests` `NornUITests` | `test(norn): add data flow and smoke coverage` |
...
- 当前实际仓库仍存在 `Domain/Legacy/Models.swift` 这个过渡文件；在 `F1` 完成前，它是允许存在的技术债，不应误判为最终结构。
- `Resources/Assets.xcassets` 已经是当前标准落点；后续不要把资源重新混回代码根目录。
- `App/NornApp.swift` 已经是当前标准落点；后续不要把入口文件重新塞回 `Application/` 或 `UI/`。
- 本文档故意不冻结后端最终字段形状；如后端 schema 重写，应在实现阶段用单独同步规范文档落地。
- 本文档也不规定 `Task` 的最终字段全集；这里只固定文件和职责拆分，不抢先锁死内部结构。
